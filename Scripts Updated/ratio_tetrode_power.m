...
%LPF 300 Hz:
%Wn1=[300/(fs_new/2)]; % Cutoff=320 Hz
...
if isempty(v_index)
    ...
    for epoch_count=1:length(v_index)
        ti_rec{epoch_count,1} = linspace(0, v_values(epoch_count), v_values(epoch_count) * fs) + v_index(epoch_count) - 1;
    end
    ti_rec = [ti_rec{:}].';
    %removing outliers from time
    if ~isempty(outliers)
        ti_recording = ti_rec(1:(outliers(1) - 1)*fs);
        for j = 1:length(outliers)-1
            ti_recording = [ti_recording ; ti_rec(outliers(j) * fs + 1 : (outliers(j+1)-1)*fs)];
        end
        ti_recording = [ti_recording ; ti_rec(outliers(end) * fs+ 1 : end)];
    else
        ti_recording = ti_rec;
    end
    ...
    parfor i = 1:length(channel)
        ...
        ti = linspace(0,length(signal)/fs_new,length(signal)).';
        thresh_power = threshold_power(fs_new,signal,window_detect,stride,5);
        ripple_all{i} = moving_window_ripples_power(channel(i),fs,raw_signal,fs_new,signal,ti_original,ti,ti_recording,thresh_power,threshold_amp);
